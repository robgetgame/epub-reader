...
## A.12 第 10 步实测记录（2026-09-12）

| 项 | 结果 |
|---|---|
| 开书（雪中悍刀行，987 文件 / 454 万字） | 0.65 s（源码）；RSS 启动 69 MB → 开书 90 MB |
| 连续切 250 章 | 切 50 章 0.72 s；RSS 92 → 93 MB（+1 MB） |
| D14 门槛（< 3 s / < 300 MB / 不增长） | 通过 |
| 真实 edge-tts 下载（7.2.8） | 通过，去重生效 |
| 真实播放：神经音色选区 3 句 | 26 s（含下载），高亮、书签、「选区读完」状态正确，无跳句 |
| 真实播放：本机 SAPI 1 句 | 通过 |
| 打包 exe（PyInstaller 6.22.2，onefile，26 MB） | 启动、开书、空格播放、预取缓存 5 文件、关窗保存书签，全部通过；截图 docs/screenshot-2026-09-12-exe.png |
| OpenRouter 真实请求 | **未测**：使用者尚未设置 OPENROUTER_KEY。传输层有 12 组假 HTTP 测试 |

未做、留给使用者的真机手动项：换音色（含播放中切换）、标点朗读听感、Ctrl+C 复制、右栏打字时空格不触发播放、
从快捷方式启动 exe、导入旧 bookmarks.json、Google Drive 同步目录。

## A.13 提供商（2026-09-12 晚）

| # | 决定 |
|---|---|
| D27 | AI 提供商可选：`config.json` `provider` = `openrouter`（默认）或 `deepseek`（直连，key 读 `DEEPSEEK_API_KEY`）。使用者先想换直连，看到价格对比后决定**先用 OpenRouter**：非高峰两家同价 $0.15 / $0.60，DeepSeek 高峰（UTC 01–04、06–10 工作日）翻倍；OpenRouter 充值收 5.5%。实测 OpenRouter 讲解首字中位 0.78 s、总 4.6 s，但 3 次里 1 次出字慢（19.7 s，疑似路由到别家）。DeepSeek 直连的真实请求**未测**（key 未设）。 |
| D28 | 追问模式允许用训练知识回答，但必须单独成段并以「以下不是本书内容，来自我的训练知识，可能有误：」开头（选项 b）。 |
| D29 | 语音预热：开章 / 恢复书签 / 停下 / 选区读完时先下好接下来 6 句；边播预取 3 → 6；章末前 8 句预热下一章开头 4 句。开章后立刻按播放仍要等第一句（1–2 s），停顿两秒再按即零等待。 |
